...
class KDTree
{
  ...
  public:
    ...
    void KDTreeRecursive(vector<Point<Dim>>& vec, int left, int right, int dimension, KDTreeNode*& parent);
    ...
    Point<Dim> findNearestNeighborHelper(const Point<Dim>& query, int dim, KDTreeNode* curRoot) const;

    double distanceFrom(Point<Dim> first, Point<Dim> second) const;
